...
\begin{enumerate}
\item For each model there is model-specific source codes \\ (\verb|$TMMROOT/models/current/mops2/src/|); Matlab script(s) in \\ \verb|$TMMROOT/models/current/mops2/matlab/| to generate input data and read model output; and run scripts and other runtime data such as namelists in runscripts/. 
...
\noin The first line defines which type of shell the script will be read. Here we will use the \verb|bash|. The second line contains the PBS resource request. The current job wull require 1 hour, 1 nodes each with 24 processor cores (ppn), and 2580 megabytes of memory per core (pmem). 
...
The follwing lines (12$\sim$) contain the commands to execute and start a specific program. Different flags need to be change accordingly depending on the nature of experiments.
\begin{lstlisting}[frame=single,basicstyle=\scriptsize,commentstyle=\color{blue}]
 ...
 # initial timestep for avg concentrations (for final year); average over 60 timesteps =1 mo
 ...
 # initial timestep for diagnostic fluxes (for final year); average over 60 timesteps =1 mo
