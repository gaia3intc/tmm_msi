...
\item Edit the C preprocessor options in \verb|MOBI_TMM_OPTION.h| as necessary. In particular, you have to define the directive \verb|O_co2_user| (i.e., by \verb|# define O_co2_ccn_user|). See Appendix \ref{subsection:mobi_options} for what each of the options mean. 
...
\item Set the path at the top of \verb|make_input_files_for_mobi_model.m| and execute in MATLAB. (See NOTE-3 in \verb|#|9.). Make sure to set the correct timestep of 28800 seconds. 
\lstset{language=matlab} 
\begin{lstlisting}[frame=single,basicstyle=\scriptsize,commentstyle=\color{blue}]
...
\begin{itemize}
\item \verb|dtts| in the \verb|&tsteps| namelist: this MUST match the time step argument to the run time option \verb|-biogeochem_deltat| in the runscript. This is the tracer advection-diffusion timestep (28800 [seconds] for UVIc model). 
...
\item Set the path at the top of the load*.m scripts to read in the output in Matlab. To make all the output netcdf files in matlab at once, run \verb|process_output.m|. Currently,the sedimentary output is suppressed. 
...
\begin{itemize}
 ...
 \item **.nc (e.g., PO4.nc): A snapshot concentration of a tracer. Edit "MOBI\_tracer\_names.txt" to select which tracers to convert to .nc files.
 \item **mmavg.nc (e.g., PO4mmavg.nc): A time-averaged concetration of a tracer. Edit "MOBI\_tracer\_names.txt" to select which tracers to convert to .nc files.
\end{itemize}
...
Figure \ref{fig:c14_mobi} compares observed and modeled radiocarbon ages. Although \ce{^14C} has not fully reached steady-state after 1000 yrs (figure not shown), the MOBI seems to reproduce observed pattern of deep-sea natural radiocarbon quite well. 
