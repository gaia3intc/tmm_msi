function write_tracers28(netcdfFileName);
...
%if useOrgCarbon
%tr10=readPetscBinVec('docout.petsc',-1);
%tr11=readPetscBinVec('pocout.petsc',-1);
...
if usePFT  
  if useORGCARBON   
    ['MOPS + (CARBON assumed) + ORGCARBON + PFT w/ or w/o FLEXCP']
    tr10=readPetscBinVec('docout.petsc',-1);
    tr11=readPetscBinVec('pocout.petsc',-1);
    tr12=readPetscBinVec('phycout.petsc',-1);
    tr13=readPetscBinVec('zoocout.petsc',-1);
    tr14=readPetscBinVec('phy2out.petsc',-1);
    tr15=readPetscBinVec('phyc2out.petsc',-1);
  else
    ['MOPS + PFT']
    tr10=readPetscBinVec('phy2out.petsc',-1);
  end
else
  if useORGCARBON
    ['MOPS + (CARBON assumed) + ORGCARBON w/ or w/o FLEXCP']
    tr10=readPetscBinVec('docout.petsc',-1);
    tr11=readPetscBinVec('pocout.petsc',-1);
    tr12=readPetscBinVec('phycout.petsc',-1);
    tr13=readPetscBinVec('zoocout.petsc',-1);
...
if usePFT  
  if useORGCARBON   
    ...
    [TR12,x,y,z]=matrixToGrid(tr12(Irr,:),[1:nb]',boxFile,gridFile);
    [TR13,x,y,z]=matrixToGrid(tr13(Irr,:),[1:nb]',boxFile,gridFile);
    [TR14,x,y,z]=matrixToGrid(tr14(Irr,:),[1:nb]',boxFile,gridFile);
    [TR15,x,y,z]=matrixToGrid(tr15(Irr,:),[1:nb]',boxFile,gridFile);
  else
    ['MOPS + PFT']
    [TR10,x,y,z]=matrixToGrid(tr10(Irr,:),[1:nb]',boxFile,gridFile);
  end
...
%OXYGEN
[data_varid3, status] = mexnc('DEF_VAR', ncid, 'O2', nc_double, 4, [time_dimid dep_dimid lat_dimid lon_dimid] );
if status, error(mexnc('STRERROR',status)), end
...
if usePFT  
  if useORGCARBON   
    ...
    [data_varid14, status] = mexnc('DEF_VAR', ncid, 'PHYTO2', nc_double, 4, [time_dimid dep_dimid lat_dimid lon_dimid] );
    if status, error(mexnc('STRERROR',status)), end
    status=mexnc('put_att_double', ncid, data_varid14, 'missing_value', nc_double', 1, NaN );
    if status, error(mexnc('STRERROR',status)), end
    
    [data_varid15, status] = mexnc('DEF_VAR', ncid, 'PHYTOC2', nc_double, 4, [time_dimid dep_dimid lat_dimid lon_dimid] );
    if status, error(mexnc('STRERROR',status)), end
    status=mexnc('put_att_double', ncid, data_varid15, 'missing_value', nc_double', 1, NaN );
    if status, error(mexnc('STRERROR',status)), end
  else
    ['MOPS + PFT']
    [data_varid10, status] = mexnc('DEF_VAR', ncid, 'PHYTO2', nc_double, 4, [time_dimid dep_dimid lat_dimid lon_dimid] );
    if status, error(mexnc('STRERROR',status)), end
    status=mexnc('put_att_double', ncid, data_varid10, 'missing_value', nc_double', 1, NaN );
    if status, error(mexnc('STRERROR',status)), end
  end
...
if usePFT  
  if useORGCARBON   
    ...
    status = mexnc('put_var_double', ncid, data_varid13, TR13 );
    if status, error(mexnc('STRERROR',status)), end
    status = mexnc('put_var_double', ncid, data_varid14, TR14 );
    if status, error(mexnc('STRERROR',status)), end
    status = mexnc('put_var_double', ncid, data_varid15, TR15 );
    if status, error(mexnc('STRERROR',status)), end
  else
    ['MOPS + PFT']
    status = mexnc('put_var_double', ncid, data_varid10, TR10 );
